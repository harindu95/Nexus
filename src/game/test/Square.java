...
public class Square extends JPanel {
	...
	private ArrayList<Player> playersOnSquare = new ArrayList<Player>();
	private Player owner = null;
	private boolean house = false;
	public String message = "";
	
	public void buyLand (Player plyr) {
		if (this.owner != null) {
			this.owner = plyr;
		}else {
			this.message = "This property belongs to someone else";
		}
	}
	
	//Precondition: user has enough remaining balance, balance updated on user object
	//post condition, update the drawing of the squre on the board
	public void buyHouse (Player plyr) {
		if (this.owner == plyr && this.house != true) {
			this.house = true;
			this.rentPrice += 30;
		}else {
			this.message = "this land is owned by someone else or already has a house";
		}
	}
	
	//need to update square
	public void landPlayer (Player plyr) {
		this.playersOnSquare.add(plyr);
	}
	
	//need to update square
	public void rmPlayer (Player plyr) {
		playersOnSquare.remove(plyr);
	}
	...
	public void paintComponent(Graphics g) {
		...
		if(this.house == true) {
			g.drawRect((this.getWidth()/2)-10, 2, 20, 16);
			g.setColor(Color.RED);
			g.fillRect((this.getWidth()/2)-10, 2, 20, 16);
		}
